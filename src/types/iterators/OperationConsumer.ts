...
export class OperationConsumer<T> implements IOpCons<T>, IOpRes<T>, IOpTr<T> {
  ...
  asyncIterator: AsyncIterable<Cursor<T>>
  ...
  map<D>(
    ...
    if (isAsyncFunction(func)) {
      return new OperationConsumer<D>(null, $mapAsync(this.iterator, func))
    } else {
  ...
  filter(
    ...
    if (isAsyncFunction(filter)) {
      return new OperationConsumer<T>(null, $filterAsync(this.iterator, filter))
    } else {
